...
Google 로그인은 방 생성과 내 방 목록·마감 관리에 사용합니다. 참여자는 공유 링크를 열면 Firebase가 브라우저별 익명 UID를 만들기 때문에 Google 로그인이나 이름 입력이 필요하지 않습니다. 같은 브라우저에서 방장이 직접 투표하면 Google 계정의 Firebase UID로 자기 선택을 저장할 수 있습니다.
...
- `ballots/current`는 Google 계정이면서 방 문서의 `ownerUid`와 현재 UID가 같은 방장만 읽기 허용
- 참여자는 마감 여부와 관계없이 투표함 읽기를 거부하고 숫자 집계를 직접 쓰거나 읽을 수 없음
- 클라이언트의 방·투표함·개별 투표 문서 삭제 거부

이 구조는 일반 웹 클라이언트끼리 개별 선택을 보지 못하게 하지만 Firebase 프로젝트 소유자·관리자가 Console이나 Admin SDK로 데이터를 확인하는 것까지 막지는 않습니다.
...
닉네임, Google 표시 이름과 이메일은 Firestore 투표 문서에 저장하지 않습니다. Firebase Authentication에는 공급자 로그인 처리를 위한 계정 정보가 존재합니다. 방장 Google 계정은 `ballots/current`의 `무작위 키 → 선택` 원본을 기술적으로 읽을 수 있고 앱 UI는 그 원본을 합계로만 변환해 표시합니다. 따라서 방장이 브라우저 개발자 도구나 Firebase SDK를 직접 사용하면 이름·UID 없는 개별 무작위 키의 선택을 볼 수 있습니다. 다만 UID가 문서 ID인 `votes` 목록은 방장도 읽을 수 없어 무작위 키를 특정 참여자 계정과 연결할 수 없습니다. 무작위 키는 `crypto.getRandomValues()`로 방마다 새로 만들며 숫자 집계를 Firestore에 따로 저장하지 않습니다.
...
3. 참여자 브라우저에서는 투표 전·후와 마감 뒤 모두 집계가 보이지 않는지 확인합니다.
4. 방장 브라우저에서는 실시간 집계가 보이고, 참여자가 자기 선택을 바꾸면 이전 항목이 1 감소하고 새 항목이 1 증가하는지 확인합니다.
5. 방장이 마감한 뒤 새 투표와 기존 선택 변경이 거절되는지 확인합니다.
6. 다른 Google 계정에서는 결과를 읽지 못하고, 같은 방장 Google 계정으로 로그인하면 **내 투표방** 목록·집계·마감 권한이 보이는지 확인합니다.
