...
test("비스표 방 만들기는 방장 로그인과 고정 8자리 명단을 받는다", () => {
  ...
  assert.match(html, /id="bisCreateWeek"[^>]*min="1"[^>]*max="99"/);
...
test("전체 현황은 11×8 표·13종 드랍·방장 주차 수정·이미지 공유를 갖춘다", () => {
  ...
  assert.match(html, /id="bisOwnerWeekInput"[^>]*min="1"[^>]*max="99"/);
