...
test("기간 시작일은 윤년·월말을 검증하고 월요일 기준 요일과 날짜 이동을 계산한다", () => {
  assert.equal(core.normalizeCalendarDate("2024-02-29"), "2024-02-29");
  assert.throws(() => core.normalizeCalendarDate("2026-02-29"));
  assert.throws(() => core.normalizeCalendarDate("2026-04-31"));
  assert.throws(() => core.normalizeCalendarDate("2026-7-20"));
  assert.throws(() => core.normalizeCalendarDate("0000-01-01"));
  assert.equal(core.addCalendarDays("2024-02-28", 2), "2024-03-01");
  assert.equal(core.addCalendarDays("2026-12-29", 6), "2027-01-04");
  assert.equal(core.addCalendarDays("2026-01-01", -1), "2025-12-31");
  assert.equal(core.calendarWeekday("2026-07-20"), 0);
  assert.equal(core.calendarWeekday("2026-07-26"), 6);
  assert.equal(core.calendarWeekStart("2026-07-22"), "2026-07-20");
  assert.equal(core.roomSlotDate("2026-07-20", 0, 0, 0, 8), "2026-07-21");
});
...
test("방 응답은 최대 8명이며 날짜를 포함해 각 UID를 기존 취합 일정으로 변환한다", () => {
  const responses = Object.fromEntries(Array.from({ length: 8 }, (_value, index) => [
    `uid-${index}`,
    validResponse(index),
  ]));
  const room = core.normalizeRoomSnapshot(validRoom({
    startDate: "2026-07-20",
    responses,
  ...
  assert.equal(schedules[0].startDate, "2026-07-20");
...
test("날짜 없는 기존 방은 계속 열리고 날짜와 저장 요일이 다른 방은 거절한다", () => {
  const legacy = core.normalizeRoomSnapshot(validRoom(), "AAECAwQFBgcICQoLDA0ODw");
  assert.equal(legacy.startDate, undefined);
  assert.equal(legacy.startDay, 0);
  assert.throws(() => core.normalizeRoomSnapshot(validRoom({
    startDate: "2026-07-22",
    startDay: 0,
  }), "AAECAwQFBgcICQoLDA0ODw"));
});
...
test("온라인 방 페이지는 noindex와 공개 설정·Firebase 모듈 로드 순서를 유지한다", () => {
  ...
  assert.match(html, /id="onlineCreateStartDate"[^>]*type="date"/);
  assert.match(html, /id="onlineCreateThisWeekButton"/);
  assert.match(html, /id="onlineCreateNextWeekButton"/);
  assert.match(html, /id="onlineCreateDateRange"/);
  assert.match(html, /id="onlineRoomPeriod"/);
...
test("Firestore Rules는 Google 방장 목록만 허용하고 9번째 응답과 타인 수정을 막는다", () => {
  ...
  assert.match(rules, /'startDate' in request\.resource\.data/);
  assert.match(rules, /!\('startDate' in data\)/);
  assert.match(rules, /data\.startDate\.matches/);
  const requiredRoomKeys = rules.match(/data\.keys\(\)\.hasAll\(\[([\s\S]*?)\]\)/)?.[1] || "";
  const allowedRoomKeys = rules.match(/data\.keys\(\)\.hasOnly\(\[([\s\S]*?)\]\)/)?.[1] || "";
  const ownerMetadataChange = rules.match(/function ownerMetadataChange\(\) \{([\s\S]*?)\n      \}/)?.[1] || "";
  assert.doesNotMatch(requiredRoomKeys, /startDate/, "기존 날짜 없는 방도 계속 검증을 통과해야 한다");
  assert.match(allowedRoomKeys, /startDate/);
  assert.doesNotMatch(ownerMetadataChange, /startDate/, "기간 시작일은 방 생성 후 바꿀 수 없어야 한다");
