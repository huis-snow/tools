...
test("날짜가 지정된 온라인 취합 이미지는 기간과 실제 날짜 열을 표시한다", async () => {
  const harness = createCanvasHarness();
  await withDocument(harness.document, () => schedule.renderComparisonImage(
    [participant("휴이스", [schedule.slotIndex(20, 0)])],
    {
      title: "날짜 있는 공대",
      timezone: "Asia/Seoul",
      startHour: 8,
  ...
  const texts = renderedTexts(harness);
  assert.match(texts.join("\n"), /기간\s+2026\.07\.20–07\.26/);
  assert.deepEqual(
    texts.filter((text) => /^\d{1,2}\/\d{1,2} [월화수목금토일]$/.test(text)).slice(0, 7),
