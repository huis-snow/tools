...
## 3. Google 로그인과 익명 로그인 켜기

1. Firebase Console에서 **Authentication → Sign-in method**로 이동합니다.
2. **Google** 공급자를 활성화하고 프로젝트 지원 이메일을 선택한 뒤 저장합니다.
3. **Anonymous(익명)** 공급자도 활성화합니다.
4. **Settings → Authorized domains**에 다음 도메인을 추가합니다.
   - 배포 사이트: `huis-snow.github.io`
   - 로컬 확인이 필요할 때: `localhost`

방을 만들고 **내 온라인 방** 목록을 보려면 Google 로그인이 필요합니다. 방 링크로 들어온 참여자에게는 로그인 화면이 나타나지 않으며, Firebase SDK가 브라우저별 익명 UID를 만들어 자기 응답만 수정할 수 있게 합니다.

기존 익명 방장이 같은 브라우저에서 Google 로그인을 누르면 익명 계정과 Google 계정을 연결해 UID와 방장 권한을 유지합니다. 이미 다른 Firebase 계정에 연결된 Google 계정으로 전환하면 기존 익명 UID의 권한은 합쳐지지 않으며 다시 복구할 수 없습니다. 현재 방에 익명 방장 권한이나 응답이 있으면 데이터 보호를 위해 전환을 중단하고, 그 밖의 경우에도 복구 불가 경고를 확인한 뒤에만 전환합니다.
...
4. **Firestore Database → Indexes → Composite**에서 다음 인덱스를 만듭니다.
   - Collection ID: `rooms`
...
저장소의 [`firebase.json`](../firebase.json)은 Firestore Rules와 [`firestore.indexes.json`](../firestore.indexes.json)의 경로를 관리하며, GitHub Pages 배포 방식에는 영향을 주지 않습니다.

보안 규칙은 다음을 서버에서 강제합니다.

- 로그인하지 않은 요청 거부
- Google 계정만 새 방 생성 허용
- Google 계정이 `ownerUid`로 소유한 최근 방만 최대 30개 목록 조회 허용
- 무작위 방 ID를 정확히 아는 사용자의 단일 방 읽기만 허용
- 필터 없는 전체 방 목록과 다른 사람의 방 목록 조회 거부
...
Google 로그인 후 방을 만들고, 다른 브라우저 프로필이나 시크릿 창에서 복사한 방 링크를 열면 익명 참여자로 테스트할 수 있습니다. 시크릿 창을 닫으면 그 익명 UID의 수정 권한을 잃을 수 있으므로 테스트 응답은 방장 브라우저에서 삭제합니다.

다른 기기에서 같은 Google 계정으로 로그인했을 때 **내 온라인 방** 목록과 방장 관리 버튼이 다시 나타나는지도 확인합니다.
...
필터 없는 방 전체 목록 조회는 보안 규칙으로 막혀 있습니다. Google 방장은 자신이 만든 최근 방만 최대 30개까지 조회할 수 있습니다. 일정이 끝난 방은 방장 화면의 **방 삭제**로 정리하고, 방장 권한이나 링크를 잃었다면 Firebase Console의 `rooms` 컬렉션에서 직접 삭제합니다.
...
rooms/{22자 무작위 roomId}
  ...
  updatedAt
  responses
    {participantUid}    익명 또는 Google 참여자 UID
      nickname
      slots
      updatedAt
```
