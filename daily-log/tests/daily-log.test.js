...
test("식단 통계는 기간별 음식 항목·기록일·빈도를 집계하고 대소문자 표기를 합친다", () => {
  ...
  const breakfast = calculateWeeklyFoodStats(state, "2026-07-01", "breakfast");
  assert.equal(breakfast.meal, "breakfast");
  assert.equal(breakfast.foodDays, 1);
  assert.equal(breakfast.mealCount, 1);
  assert.equal(breakfast.itemCount, 2);
  assert.equal(breakfast.uniqueItemCount, 2);
  assert.deepEqual(breakfast.items.map((item) => item.name).sort(), ["김치", "밥"]);

  const dinner = calculateMonthlyFoodStats(state, "2026-07", "dinner");
  assert.equal(dinner.foodDays, 1);
  assert.equal(dinner.itemCount, 2);
  assert.deepEqual(dinner.items.map((item) => item.name).sort(), ["coffee", "김치"]);
  assert.throws(
    () => calculateFoodStats(state, { from: "2026-07-02", to: "2026-07-01" }),
    /시작일/,
  );
  assert.throws(
    () => calculateWeeklyFoodStats(state, "2026-07-01", "brunch"),
    /끼니 구분/,
  );

  const repeatedState = createEmptyState();
  upsertRecord(repeatedState, "2026-07-01", fullRecord({
    meals: { breakfast: "밥", lunch: "", dinner: "밥", snack: "" }, condition: 0, memo: "",
  }), fixedNow);
  const repeatedAll = calculateWeeklyFoodStats(repeatedState, "2026-07-01");
  assert.deepEqual(repeatedAll.items[0], { name: "밥", count: 2, dayCount: 1 });
  assert.deepEqual(
    calculateWeeklyFoodStats(repeatedState, "2026-07-01", "breakfast").items[0],
    { name: "밥", count: 1, dayCount: 1 },
  );
});
...
function createDom(storedState = null) {
  const ids = [
    ...
    "drinkingDaysCount", "averageCondition", "mealStatsWeekButton", "mealStatsMonthButton",
    "mealStatsAllButton", "mealStatsBreakfastButton", "mealStatsLunchButton", "mealStatsDinnerButton",
    "mealStatsSnackButton", "mealStatsRange",
...
test("DOM 초기화는 오늘을 선택하고 월요일 시작 42일 달력과 빈 통계를 그린다", () => {
  ...
  assert.equal(env.elements.get("mealStatsRange").textContent, "주간 · 2026년 7월 13일–19일 · 전체 식사");
...
test("입력 변경은 선택 날짜에 즉시 저장하고 달력·통계를 갱신한다", async () => {
  ...
  await env.elements.get("mealStatsLunchButton").click();
  assert.equal(env.elements.get("mealStatsLunchButton").getAttribute("aria-pressed"), "true");
  assert.equal(env.elements.get("mealStatsAllButton").getAttribute("aria-pressed"), "false");
  assert.equal(env.elements.get("mealStatsRange").textContent, "주간 · 2026년 7월 13일–19일 · 점심");
  assert.equal(env.elements.get("mealStatsItems").textContent, "0");
  assert.equal(env.elements.get("mealRanking").hidden, true);
  assert.equal(env.elements.get("mealStatsEmpty").hidden, false);
  assert.match(env.elements.get("mealStatsEmpty").textContent, /점심/);
  await env.elements.get("mealStatsMonthButton").click();
  assert.equal(env.elements.get("mealStatsLunchButton").getAttribute("aria-pressed"), "true");
  assert.equal(env.elements.get("mealStatsRange").textContent, "월간 · 2026년 7월 · 점심");
  await env.elements.get("mealStatsWeekButton").click();
  await env.elements.get("mealStatsBreakfastButton").click();
  assert.equal(env.elements.get("mealStatsItems").textContent, "2");
  assert.equal(env.elements.get("mealRanking").hidden, false);
  await env.elements.get("mealStatsAllButton").click();
  await env.elements.get("mealStatsMonthButton").click();
  ...
  assert.equal(env.elements.get("mealStatsRange").textContent, "월간 · 2026년 7월 · 전체 식사");
